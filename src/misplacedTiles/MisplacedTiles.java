...
//	int counter = 1;
	...
	public void search()
	{
		while(!Play.goalFound)
		{
			...
			checkGoal = countTiles(Play.stateMapping.get(exploring));
			if(checkGoal == 0)
			Play.goalFound = true;
	...
	private void swap(int iTargetPos, int jTargerPos, int iPos, int jPos) 
	{
		...
		System.out.println("Misplaced tiles--> "+countTiles(currentState));
		Play.childParent.put(Play.stateCounter, exploring);
		Play.stateTilesScore.put(Play.stateCounter, countTiles(currentState));
	...
	public static int countTiles(char[][] cs) 
	{
		int counter = 0;
		for (int i = 0; i < 3; i++) 
		{
